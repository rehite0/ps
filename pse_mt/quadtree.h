...
int qt_insert_mt(_point* p,qtree* t);
int qt_pvt_insert_leaf_mt(_point* p, qtree* t);
...
int qt_pvt_insert_leaf_mt(_point* p, qtree* t){
	static _point* use[num_thread]={0};
	qtree* rt=t;
	//lock
	//check if in use
	...
	switch (t->len){
		...
		case 4:
			...
			goto exit;
		default:
			assert(0 &&"unable to insert unexpected behaviour!");
	}//switch
...
// | |_o-p,p,p,p
// | ^
// | ----can be operated by simple non-multi threaded fn as other thread are block by lock parent
// lock
//increment len at last
//so other see it as leaf until operation is complete
//tbh i have no idea how and why this work, but the fact is `it just does`
...
int qt_insert_mt(_point* p, qtree* t){
	...
	switch (t->len){
		...
		case 3:
		case 4:
			while(!qt_pvt_insert_leaf_mt(p,t));
			retutn 1;
		default:
			++(t->len);
			break;
	}//switch
	return qt_insert_mt(p,t->q1.t)
		|| qt_insert_mt(p,t->q2.t)
		|| qt_insert_mt(p,t->q3.t)
		|| qt_insert_mt(p,t->q4.t);
}//fn
